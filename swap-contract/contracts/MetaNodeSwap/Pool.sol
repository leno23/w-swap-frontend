...
contract Pool is IPool {
    ...
    // feeGrowth 手续费增长量
    // Global 整个池子层面
    // 0 代表 token0，1 代表 token1
    // X128 表示 128 位定点数，用于表示小数
    
    // 记录了从池子创建到现在，每单位流动性累计产生的 token0 手续费总量。
    uint256 public override feeGrowthGlobal0X128;
    /// @inheritdoc IPool
    uint256 public override feeGrowthGlobal1X128;

    struct Position {
        // 该头寸 即Position 拥有的流动性
        uint128 liquidity;
        // 可提取的 token0 数量
        uint128 tokensOwed0;
        // 可提取的 token1 数量
    ...
    function _modifyPosition(
        ...
        // 用户应得手续费 =
        //  (当前全局累计值 - 用户上次提取时的累计值) × 用户流动性 ÷ Q128
    ...
    function balance0() private view returns (uint256) {
        ...
        require(success && data.length >= 32, "Failed to get token0 balance");
    ...
    function balance1() private view returns (uint256) {
        ...
        require(success && data.length >= 32, "Failed to get token1 balance");
        return abi.decode(data, (uint256));
    }
